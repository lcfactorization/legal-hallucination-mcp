...
class LegalHarness:
    ...
    def load_and_compile(self) -> list[str]:
        ...
        logger.info(
            "load_and_compile: 诉请解析完成, claim_limits=%s",
            json.dumps(self.claim_limits, ensure_ascii=False),
        )
    ...
    def verify_strict_scope_containment(self) -> list[str]:
        ...
        for v in violations:
            if v.violation_type == "项目越权":
                errors.append(
                    f"越权裁判：判决主文支持了未曾主张的项目"
                    f"「{v.judgment_item} {v.judgment_amount}元」，违反不告不理原则"
                )
